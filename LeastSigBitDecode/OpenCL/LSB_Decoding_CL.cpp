...
//print binary array
void printBinaryArray(int* binary, int binarySize) {
...
int main(void)
{
    ...
    cl_kernel kernel = clCreateKernel(program, "decodeImage", &err);
    ...
    size_t global_size = binarySize; //total number of work items (since its a square matrix its eg 4x4)
    ...
    image_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int)*imageSize, image, &err); //check later!!!!
	binary_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(int)*binarySize, binary, &err);
    ...
    clSetKernelArg(kernel, 2, sizeof(int), (void*)&binarySize); //check later !!!
	...
	err = clEnqueueReadBuffer(queue, binary_buffer, CL_TRUE, 0, sizeof(binary), binary, 0, NULL, NULL);
    ...
    //TODO: Verify Binary
	//cout << "Binary array: ";
    //printBinaryArray(binary, binarySize);
